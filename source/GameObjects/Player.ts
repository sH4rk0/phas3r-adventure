...
module z89 {
    ...
    export class Player extends Phaser.GameObjects.Sprite {
        ...
        constructor(scene: GameCity) {
            ...
            config = {
                key: 'beam',
                frames: this.scene.anims.generateFrameNumbers('beam',{ start: 0, end: 11 } ),
                frameRate: 15,
                repeat: -1,
                repeatDelay: 0
            };
           ...
           /* this.animations.add("walk", [0, 1, 2, 3, 4, 5, 6, 7], 7, true);
            ...
            this.setOrigin(0.5, 1).setScale(1).setY(608).setInteractive().setName("player").on('pointerdown',()=>{
...
/*
   ...
   */         
        ...
        goTo(_x: number, _y: number, _item?: Items): void {
            ...
            this.hideBaloon();
            if(this.scene.currentItem == undefined && _item == undefined) this.scene.playerActions.hide();

            if (this.scene.playerActions.IsOpen() && this.scene.currentItem != undefined && _item != undefined && this.scene.currentItem.itemObj.id != _item.itemObj.id) this.scene.playerActions.hide();

            if (this.scene.conversationBaloon.isConversationActive() && (_x != this.x || _y != this.y - 5)) { this.scene.conversationBaloon.stopConversation() }
        ...
        checkIntersect(_toPosition: any): any {
            ...
            let line1: Phaser.Geom.Line = new Phaser.Geom.Line(_toPosition.x1, _toPosition.y1, this.x, this.y)
            let line2: Phaser.Geom.Line;
            let intersectPoint: Phaser.Geom.Point=new Phaser.Geom.Point();


            this.scene.getSprites().children.each((sprite) => {
        ...
        public blinkTo(_x: number) {
            ...
            if (this.scene.conversationBaloon.isConversationActive()) { this.scene.conversationBaloon.stopConversation() }
        ...
        beamIn(toX: number) {
            ...
            let beam: Phaser.GameObjects.Sprite = this.scene.add.sprite(toX, 0, "beam");
            ...
            let tweenBeam: Phaser.Tweens.Tween = this.scene.tweens.add({
                ...
                onComplete:()=>{
                    ...
                    this.scene.tweens.add({ targets:this,alpha:1,duration:300, repeat:0});
                    this.scene.tweens.add({ targets:beam,alpha:0,duration:300, repeat:0});
        ...
        beamH():number{  return this.y/200;  }

        beamOut(toX: number) {


            let beam: Phaser.GameObjects.Sprite = this.scene.add.sprite(this.x, 0, "beam");
            beam.setScale(.5,this.beamH()).setOrigin(.5, 0).setAlpha(0).play("beam");
            ...
            let test: Phaser.Tweens.Tween=this.scene.tweens.add({
                ...
                onComplete:()=>{
                    ...
                    this.scene.tweens.add({targets:this, y:this.y-100, scaleX: .25, scaleY: 10, alpha: 0, duration:300,ease: 'Power1', repeat:0,
                    onComplete:() => {
        ...
        public showBaloon(_text: string) { this.scene.playerBaloon.showBaloon(_text); }
        public showBaloonExtra(_obj: any) { this.scene.playerBaloon.showBaloonExtra(_obj); }
        public hideBaloon() { this.scene.playerBaloon.hideBaloon(); }
