namespace z89 {
  ...
  export class TerminalLogic {
    ...
    private lettersObj: Array<Phaser.GameObjects.BitmapText> = [];
    ...
    private cursorOffsetX: number = 228;
    private cursorOffsetY: number = 98;

    constructor(scene: GameCity, terminal: Terminal, tint?: number) {
      this.scene = scene;
      ...
      for (var i = 0; i < 25; i++) {
        this.lettersObj.push(
          this.scene.add.bitmapText(220, 16 * i + 90, "commodore", "", 16)
        );
    ...
    setCursorX(index: number): number {
      return index * 16 + this.cursorOffsetX;
    }

    setCursorY(index: number): number {
    ...
    returnCredits(): Array<any> {
      return this.returnStaticString(msgs.credits, 0);
    }
    returnProcessing(): Array<any> {
      return this.returnStaticString(msgs.processing, 0);
    }

    returnError(error?: string): Array<any> {
      let _error: string = this.errors[
        Phaser.Math.RND.integerInRange(0, this.errors.length - 1)
      ];
    ...
    removeChar(): void {
      ...
      if (this.isShell) {
        if (col > this.shellStart) {
          this.lettersObj[row].text = this.replaceAt(
            this.lettersObj[row].text,
            col - 1,
            " "
          );
          this.cursor.x = this.setCursorX(col - 1);
    ...
    addChar(key: string): void {
      ...
      this.lettersObj[row].setText(
        this.replaceAt(this.lettersObj[row].text, col, key)
      );
      this.cursor.x = this.setCursorX(col + 1);
      col++;
      if (col == 40) {
    ...
    sendOrder(itemId: number): void {
      ...
      this.scene.time.delayedCall(
        ...
        () => {
          ...
          this.scene.gameItemsUtils.beamIn(
            this.scene.gameItemsUtils.getItemById(13)
          );
        },
        null,
        this
          ...
          callback: () => {
            ...
            if (i == 5)
              this.scene.time.addEvent({
                ...
                callback: () => {
                  this.terminal.hide();
                  this.terminal.scene.shootFromHigh([27]);
                },
                callbackScope: this
              });
          },
    ...
    write(obj: any, cursorNext: boolean = true): void {
      let element: any = obj;
      let textObj: Phaser.GameObjects.BitmapText;
      let cDelay: number = 0;
      let row: number = this.getCursorRow();
      if (row == 25) {
        this.scrollDown();
