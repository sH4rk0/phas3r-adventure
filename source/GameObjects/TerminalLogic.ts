module z89 {
    ...
    export class TerminalLogic {
        ...
        returnReady(txt?: string): any {
            ...
            if (txt != undefined) { _ready.text = txt; this.cursor.x = this.setCursorX(txt.length); }
            this.cursor.play("blink");
            return _ready;
        ...
        returnError(error?: string): Array<any> {
            ...
            let _error: string = this.errors[Phaser.Math.RND.integerInRange(0, this.errors.length - 1)];
        ...
        charUp(): void {
            ...
            if (row > 0) { this.cursor.y = this.setCursorY(row-1); }
        ...
        charDown(): void {
            ...
            if (row < 24) { this.cursor.y = this.setCursorY(row+1);  } else {
        ...
        hitTarget(): void {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                this.scene.time.addEvent({ delay: 1000 * i, callback: ()=>{
                    ...
                    if (i == 5) this.scene.time.addEvent({delay:1000, callback:() => {
                        this.terminal.hide();
...
//console.log(xhr)
        ...
        write(obj: any, cursorNext: boolean = true): void {
            ...
            if (cursorNext) {
                ...
                if (row == 25) { this.scrollDown(); this.cursor.y = this.setCursorY(row-1); }
            }
